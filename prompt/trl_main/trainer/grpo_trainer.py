...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # 课程采样相关超参（若 args 中没有则给默认值）
        self.use_curriculum_sampler = getattr(args, "use_curriculum_sampler", True)
        self.curriculum_sigma = getattr(args, "curriculum_sigma", 0.25)
        self.curriculum_lambda = getattr(args, "curriculum_lambda", 0.1)
        self.curriculum_num_buckets = getattr(args, "curriculum_num_buckets", 5)
        self.curriculum_sampler = None  # initialized later in _get_train_sampler()
    ...
    def _generate_with_retry(self, prompts, inputs):
        ...
        print(f'<_generate_with_retry> First texts: {texts}')
        ...
        # # 50% 概率重试
        # retry_idx = [i for i in retry_idx_all if random.random() <= 0.75]
        retry_idx = retry_idx_all
        ...
        for i in retry_idx:
            ...
            refine = "Your answer was incorrect. Try again. Provide reasoning and the final answer (A, B, C, D, or E) enclosed within \\boxed{},for example, \\boxed{A}, \\boxed{B}, \\boxed{C}, \\boxed{D}, or \\boxed{E}."
        ...
        print(f'<_generate_with_retry> Retry prompts: {retry_prompts}')
        ...
        def safe(obj):
            ...
            if "PIL" in str(type(obj)):
                return str(obj)  # 或者 obj.filename
            return obj

        save_dict = {k: safe(v) for k, v in save_dict.items()}

        with open("conversation_vars.jsonl", "a", encoding="utf-8") as f:
