...
# hard to easy
def reverse_difficulty(example):
    example["difficulty"] = 1.0 - example["difficulty"]
    return example
train_dataset = train_dataset.map(reverse_difficulty,num_proc=32)

print(train_dataset[0]["prompt"])
print(train_dataset[0]["image"])
print(train_dataset[0]["difficulty"])
...
def accuracy_reward(completions, **kwargs):
    ...
    for text, org_ground_truth in zip(texts, ground_truth):
        # print(text)
        new_answer = BOXED_RE.search(text)
        # if new_answer:
        #     print("new_ansewr: ", new_answer.group(1))
        #     print("ground_truth: ", org_ground_truth)
        if new_answer and new_answer.group(1).upper() == org_ground_truth.upper():
            rewards.append(1)
        else:
            rewards.append(0)
...
trainer.train(resume_from_checkpoint= "/home/dangyunkai/yunkai/VLM/VIG-Group/jiarui/HARI_hardtoeasy/HARI/checkpoint-42270")
