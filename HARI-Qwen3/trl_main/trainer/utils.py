...
from torch.utils.data import Sampler
import torch
from typing import Sequence, Sized, Iterator
...
# class CurriculumSampler(Sampler[int]):
#     """
#     动态课程序采样：权重 w = exp(-(d - t)^2/(2*sigma^2)) + lambda * |Δb|
#     - difficulties: Tensor(shape=[N])，归一化难度 d
#     - buckets: LongTensor(shape=[N])，0..num_buckets-1
#     """
#     def __init__(
#         self,
#         data_source: Sized,
...
#         mini_repeat_count: int = 1,
#         repeat_count: int = 1,
#         replacement: bool = False, #不放回抽样
#         generator: torch.Generator | None = None,
#         shuffle: bool = True,
#         seed: int | None = None,
#     ):
#         if len(difficulties) != len(data_source) or len(buckets) != len(data_source):
#             raise ValueError("difficulties / buckets size must match dataset size")
#         self.data_source = data_source
#         self.difficulties = difficulties.double()
#         self.buckets = buckets.long()
#         self.batch_size = batch_size
...
#     def set_step(self, step: int):
#         """更新当前进度 t 并重算权重（不采样，外部每步调用）"""
#         self.step = step
#         t = min(max(step / max(1, self.total_steps), 0.0), 1.0)
#         p_td = torch.exp(- (self.difficulties - t) ** 2 / (2 * self.sigma ** 2))
#         delta_b = torch.abs(self.bucket_now - self.bucket_prev)
#         bucket_boost = delta_b[self.buckets]
#         self.weights = p_td + self.lambda_ * bucket_boost
#         # 避免全零
#         self.weights = torch.clamp(self.weights, min=1e-8)

#     def update_bucket_stats(self, bucket_correct: torch.Tensor, bucket_total: torch.Tensor):
#         """桶级正确率更新：先把 now 备份到 prev，再用新的统计更新 now"""
#         self.bucket_prev = self.bucket_now.clone()
#         bucket_correct = bucket_correct.to(dtype=torch.double)
#         bucket_total = bucket_total.to(dtype=torch.double)
#         self.bucket_correct_cum += bucket_correct
#         self.bucket_total_cum += bucket_total
#         mask = self.bucket_total_cum > 0
#         new_rates = torch.zeros_like(self.bucket_now)
#         new_rates[mask] = self.bucket_correct_cum[mask] / self.bucket_total_cum[mask]
#         self.bucket_now = torch.clamp(new_rates, min=0.0, max=1.0)
#         print(f"Updated bucket accuracies: {self.bucket_now.tolist()}")

#     def __iter__(self) -> Iterator[int]:
#         num_chunks = len(self.data_source) // self.batch_size
#         if num_chunks == 0:
#             return iter([])
                
#         if self.used_mask is None or self.used_mask.all():
#             self.used_mask = torch.zeros(len(self.data_source), dtype=torch.bool)
#             print(f"[sampler] reset used_mask, total={len(self.data_source)}")
...
#         chunks: list[list[int]] = []
#         for _ in range(num_chunks):
#             if local_weights.sum() <= 0 or (local_weights > 0).sum() < self.batch_size:
#                 break
#             batch_idx = torch.multinomial(
#                 local_weights,
...
#             self.used_mask[batch_idx] = True
#             local_weights[batch_idx] = 0.0
#         # chunks = [sampled[i:i + self.batch_size] for i in range(0, len(sampled), self.batch_size)]
#         # for chunk in chunks:
#         #     for _ in range(self.repeat_count):
#         #         for idx in chunk:
#         #             for _ in range(self.mini_repeat_count):
...
#     # def __len__(self) -> int:
#     #     total = len(self.data_source)
#     #     num_chunks = (total + self.batch_size - 1) // self.batch_size  # ceil
#     #     return num_chunks * self.batch_size * self.mini_repeat_count * self.repeat_count
...
class WeightedRepeatSampler(Sampler[int]):
    """
    类似 RepeatSampler + WeightedRandomSampler。
    """
    def __init__(
        self,
        data_source: Sized,
        ...
        if len(weights) != len(data_source):
            raise ValueError("weights size must match dataset size")
        self.data_source = data_source
    ...
    def __iter__(self) -> Iterator[int]:
        # 需要的 chunk 数 = floor(num_samples / batch_size)
        num_chunks = self.num_samples // self.batch_size
        if num_chunks == 0:
            return iter([])

        # 按权重抽取 chunk_size * num_chunks 个索引
        draw_count = num_chunks * self.batch_size
...
class RepeatSampler(Sampler):
    ...
    def __iter__(self):
        ...
        #             for _ in range(self.mini_repeat_count):
        #                 yield index
        for batch_id, chunk in enumerate(indexes):
            print(f"[sampler] batch={batch_id}, idx={chunk}")
            for _ in range(self.repeat_count):
                for idx in chunk:
                    for _ in range(self.mini_repeat_count):
                        yield idx
