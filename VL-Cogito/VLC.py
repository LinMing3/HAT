...
# -------------------- load dataset -------------------- #
dataset_dir = "/home/dangyunkai/yunkai/VLM/VIG-Group/jiarui/VL-Cogito/dataset_filtered_qwen25vl7b_50acc"
...
train_dataset = train_dataset.map(make_conversation, num_proc=8,writer_batch_size=64)
...
ODSW_FNS = {
    "easy":  lambda acc: torch.where(acc <= 0.5, torch.sin(math.pi * acc), torch.ones_like(acc)),
    "medium":lambda acc: torch.sin(math.pi * acc),
    "hard":  lambda acc: torch.where(acc <= 0.5, torch.ones_like(acc), torch.sin(math.pi * acc)),
}

class GRPOVLcogitoTrainer(GRPOTrainer):

    def __init__(
        self,
        *args,
        stage_name: str = "default",
        odsw_weight_fn: Optional[Callable[[torch.Tensor], torch.Tensor]] = None,
        **kwargs,
    ) -> None:
        super().__init__(*args, **kwargs)
        self.stage_name = stage_name
        self.odsw_weight_fn = odsw_weight_fn or self._identity_weight_fn

    @staticmethod
    def _identity_weight_fn(acc: torch.Tensor) -> torch.Tensor:
        return torch.ones_like(acc)
    ...
    def set_difficulty(
        ...
        self.stage_name = stage_name
        self.odsw_weight_fn = weight_fn or self._identity_weight_fn

    def _prepare_multimodal_prompts(
        self, inputs: list[dict[str, Any]], prompts: list[Any]
    ) -> tuple[list[Any], Optional[list[Any]], dict[str, Any], Optional[list[int]], bool]:
        kwargs: dict[str, Any] = {}
        images: Optional[list[Any]] = None
        image_split_sizes: Optional[list[int]] = None
        has_images = False
        ...
        return prompts, images, kwargs, image_split_sizes, has_images
    ...
    def _generate_token_ids(
        ...
        kwargs: dict[str, Any],
        image_split_sizes: Optional[list[int]],
        has_images: bool,
        device: torch.device,
        images: Optional[list[Any]],
    ) -> tuple[torch.Tensor, Optional[torch.Tensor], torch.Tensor, torch.Tensor, Optional[list[int]]]:
        sampling_per_token_logps: Optional[torch.Tensor] = None
        ...
        if self.use_transformers_paged:
            ...
            with profiling_context(self, "transformers.generate_batch"), \
            ...
            completion_ids = [torch.tensor(ids, device=device) for ids in all_outputs.values()]
        ...
        return completion_ids, sampling_per_token_logps, prompt_ids, prompt_completion_ids, image_split_sizes
    ...
    #     prompts, images, kwargs, image_split_sizes, has_images = self._prepare_multimodal_prompts(inputs, prompts)
    ...
    #     completion_ids, sampling_per_token_logps, prompt_ids, prompt_completion_ids, image_split_sizes = (
    ...
    #     # Apply ODSW weights if specified
    #     if self.odsw_weight_fn is not None:
    #         group_rewards = rewards.view(-1, self.num_generations)
    #         acc = group_rewards.mean(dim=1)
    #         weights = self.odsw_weight_fn(acc)
    #         print(f"ODSW weights ({self.stage_name}): ", weights)
    #         weights = weights.repeat_interleave(self.num_generations)
    ...
    def _generate_and_score_completions(
        ...
        print("rewards_per_func: ", rewards_per_func.tolist())
        ...
        if self.odsw_weight_fn is not None:
            group_rewards = rewards.view(-1, self.num_generations)
            acc = group_rewards.mean(dim=1)
            weights = self.odsw_weight_fn(acc).to(advantages.device)
            print(f"ODSW weights ({self.stage_name}): ", weights)
            weights = weights.repeat_interleave(self.num_generations)
            advantages = advantages * weights
            self._metrics[mode][f"odsw/{self.stage_name}/mean"].append(weights.mean().item())
...
# group_rewards = rewards.view(-1, self.num_generations)
# acc = group_rewards.mean(dim=1)              # 每个 prompt 的 rollout accuracy
# weights = self.odsw_weight_fn(acc)           # 选定 ODSW 版本
# weights = weights.repeat_interleave(self.num_generations)
# advantages = advantages * weights
...
# --
STAGES = [
    {"name": "easy",   "fn": ODSW_FNS["easy"]},
    {"name": "medium", "fn": ODSW_FNS["medium"]},
    {"name": "hard",   "fn": ODSW_FNS["hard"]},
]

# -------------------- 续训配置 -------------------- #
resume_checkpoint_path = "/home/dangyunkai/yunkai/VLM/VIG-Group/jiarui/VL-Cogito/VLC/checkpoint-6322"
resume_stage_name = "hard"  # None 表示从 easy 开始全量训练

stage_name_to_idx = {stage["name"]: idx for idx, stage in enumerate(STAGES)}
if resume_stage_name is not None:
    if resume_stage_name not in stage_name_to_idx:
        raise ValueError(f"未找到阶段 {resume_stage_name}，请检查 STAGES 定义")
    start_stage_idx = stage_name_to_idx[resume_stage_name]
else:
    start_stage_idx = 0

best_ckpt = resume_checkpoint_path

# -------------------- 训练 -------------------- #
trainer = GRPOVLcogitoTrainer(
    model=model,
...
for stage_idx, stage in enumerate(STAGES):
    ...
    trainer.set_difficulty(stage["name"], stage["fn"])
    trainer.train_dataset = train_dataset.shuffle(seed=stage_idx)

    trainer.args.num_train_epochs = stage_idx + 1        # total epochs seen so far
    trainer.args.max_steps = -1                        # let HF recompute from epochs
