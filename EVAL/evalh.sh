...
RESULT_SAVE_PATH="/home/yangjiacheng/data/jiarui/EVAL/Qwen-HR8k"


# CKPT_ROOT="/home/yangjiacheng/data/jiarui/GRPO/TRAIN/GRPO-more"
# LOG_ROOT="/home/yangjiacheng/data/jiarui/GRPO/EVAL-all"
# RESULT_SAVE_PATH="/home/yangjiacheng/data/jiarui/EVAL/GRPO-HR8k"

LOG_DIR="${LOG_ROOT}/HR8k-logs"
mkdir -p "$LOG_DIR"
mkdir -p "$RESULT_SAVE_PATH"
TS="$(date +%Y%m%d_%H%M%S)"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HR8k_PY="$SCRIPT_DIR/HR8k.py"
...
for ckpt in "$CKPT_ROOT"/checkpoint-*; do
  ...
  found_any=1
  log_file="$LOG_DIR/checkpoint-${step}_${TS}_HR8k.log"

  echo "==> checkpoint-${step} | log: ${log_file}"
  CUDA_VISIBLE_DEVICES="0" TRAINED_MODEL_ID="$ckpt" RESULT_SAVE_PATH="$RESULT_SAVE_PATH" python "$HR8k_PY" >"$log_file" 2>&1
``
done
